...
def predict_videos(model, net, videos, videos_path, args):
    ...
    for video in videos:
        ...
        try:
            ...
            for item in range(len(frames_filenames)):
                prediction = model.infer(inputs={input_blob: frames[item]})
                prediction = prediction[out_blob]

                if prediction[0][0] > 0.5:
                    prob = round(100 * prediction[0][0], 2)
                    label = "Non-Violence: " + str(prob) + ' %'
                    # text color
                    red = 0
                    green = 255
                    csv_data.append(frames_filenames[item].split('/')[-1] + ';' 
                        + 'Non-Violence' + ';' + str(prediction[0][0]))
                else:
                    prob = round(100 * (1. - prediction[0][0]), 2)
                    label = "Violence: " + str(prob) + ' %'
                    # text color
                    red = 255
                    green = 0
                    csv_data.append(frames_filenames[item].split('/')[-1] + ';' + 
                        'Violence' + ';' + str(1. - prediction[0][0]))
